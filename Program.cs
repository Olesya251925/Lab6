...
namespace lab
{
    ...
    class Event1 : IEvent
    {
        public Event1() { EventType = "Диагональ матрицы:"; }
    }
    class Event2 : IEvent
    {
        public Event2() { EventType = "Не удалется получить диагональ матрицы"; }
    }
    public abstract class BaseHandler
    {
        ...
        public virtual void Handle(IEvent ev)
        {
            ...
            {
                Console.WriteLine("{0} Обработано успешно", PrivateEvent.EventType);
            }
            else
            {
                Console.WriteLine("Отправка события ");
                if (Next != null)
                    Next.Handle(ev);
                else
